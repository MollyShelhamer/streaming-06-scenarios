"""src/streaming/kafka_consumer_shelhamer.py.
...
OUTPUT_FIELDNAMES: Final[list[str]] = [
    *CONSUMED_FIELDNAMES[:-3],
    "eligible_discount_code",
...
def process_message(
    ...
    *,
    ...
    if str(row.get("is_new_customer", "")).strip().lower() == "true":
        enriched["eligible_discount_code"] = "WELCOME10"
        enriched["subtotal_with_discount"] = round(enriched["subtotal"] * 0.9, 2)
    else:
        enriched["eligible_discount_code"] = ""
        enriched["subtotal_with_discount"] = enriched["subtotal"]

    LOG.info(
        f"subtotal={enriched['subtotal']}  "
        f"eligible_discount_code={enriched['eligible_discount_code']}  "
        f"subtotal_with_discount={enriched['subtotal_with_discount']}  "
